...
\hyphenation{OsmAnd}
\thesislong{abstract}{
Being a visually impaired person poses many challenges one of which is usage of navigation applications.
This thesis first explores the currently available applications for mobile operating systems and desktops in terms of their accessibility and provided features.
The next chapter introduces Feel the Streets, an application which intents to help visually impaired people in exploring new areas.
The following chapter then presents a survey within the group of visually impaired people which ries to answer the question how usable the application actually is.
And the last one presents conclusions and possible future directions.
}
\thesissetup{advisor={Ing.\,Milan Brož,\,Ph.D.,\,RNDr.\,Agáta Kružíková}}
\thesislong{thanks}{
I would like to thank all my thesis advisors for their beneficial tips and for their time in general. Namely, my thanks go to Bohdan Milar who spend immense amounts of time going through almost any of the many application versions with a lot of bugs. I would also like to thank Agáta Kružíková, who helped immensely with formulation with the research questions and with all the required methodology and other formalities which bring the research up-to the required standards. I would also like to thank all the testers, and, with his permission, special thanks go to Martin Jozífek, who helped track many nasty issues and dedicated a lot of his time which allowed the application to be in much better shape than it was before.
}
\thesisload
...
From the user's perspective, Feel the Streets is a cross-platform desktop application for Windows and Linux operating systems that, when launched, allows the user to browse OpenStreetMap maps without sight.
...
For the sound engine, the client application uses OpenAL and the HRTF (head-related transfer function) extension for more realistic 3D audio positioning. That means that the OpenAL Soft library \parencite{openalsoft} is required.
...
In commit 62a1454247, the framework for executing actions on objects was introduced. Some commits later, the Wikipedia and wiki data open actions were implemented along with other website-like things which could be opened, like the RUIAN (the Czech land register) details of a Czech building.
...
This section describes the other attachments in the thesis archive. The file \emph{sus.xlsx} contains the results of the System usability scale survey along with the calculated scores and the relevant formulas. The file \emph{dp\_subject\_mail.txt} contains text of the email which was sent to the participants, note that the text is only in the Czech language and is highly informal, because I personally knew all of them. Also in Czech, the file \emph{user\_docs.html} contains the documentation which was available to the survey participants. To help all the future readers, the file \emph{fts\_app.zip} is the Windows build of Feel the Streets used for testing. And, to allow introspection of the source code at that time, the file \emph{fts\_src.zip} contains the source code as of commit f316b17d63.
\end{document}